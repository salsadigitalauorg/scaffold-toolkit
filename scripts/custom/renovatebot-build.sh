...
if [ -n "${DREVOPS_DB_DOCKER_IMAGE:-}" ]; then
  ...
  ./scripts/drevops/restore-docker-image.sh "${DREVOPS_DB_DOCKER_IMAGE}" "${DREVOPS_DB_DIR}/db_renovate.tar"
  # If the image does not exist and base image was provided - use the base
  # image which allows "clean slate" for the database.
  # LCOV_EXCL_START
  if [ ! -f "${DREVOPS_DB_DIR}/db_renovate.tar" ] && [ -n "${DREVOPS_DB_DOCKER_IMAGE_BASE:-}" ]; then
...
docker compose exec ${dcopts[@]} cli bash -c "./scripts/custom/renovatebot-provision.sh"
echo

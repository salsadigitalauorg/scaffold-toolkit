...
class ScaffoldInstaller {
    ...
    private string $baseUrl = 'https://raw.githubusercontent.com/salsadigitalauorg/scaffold-toolkit/main';
    ...
    private function processFile(array $file): void {
        ...
        if ($this->dryRun) {
            echo "[DRY RUN] Would download {$sourceFile} to {$targetFile}\n";
            return;
        }

        $content = $this->downloadFile($sourceFile);
        if ($content === false) {
            throw new \RuntimeException("Failed to download {$sourceFile}");
        }
     ...
     */
    private function downloadFile(string $path): string|false {
...
$options = getopt('', ['latest', 'version:', 'dry-run', 'force', 'ci:', 'hosting:']);
